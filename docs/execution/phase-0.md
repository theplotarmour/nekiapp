...
| NK-DOC-4.02 | Reviewed OpenAPI schemas/examples, validation and generated-client workflow; reconcile TDD/schema references. | NK-DOC-4.01 | Backend / split after inventory | In progress: identity/preference slice typed and validated; remaining operations tracked explicitly |
...
| OpenAPI/errors/auth matrix | Partial: 324 operation rows spanning all 22 plan families, route/field policies and request/error profiles. Structural validator passes. First 25 identity/preference operations typed in OpenAPI with 263 validated examples and 23 rejected negative schema cases; 299 operations remain untyped. Final grants, source reconciliation, runtime checks and full per-operation event schemas missing. |
