...
| OpenAPI/errors/auth matrix | Partial: 324 operation rows spanning all 22 plan families, route/field policies and request/error profiles. Structural validator passes. 119 identity/discovery/organization/mission-management and moderation operations typed in OpenAPI with 1,242 validated examples, 61 rejected negative schema cases and five rejected query cases; 205 operations remain untyped. Final grants, source reconciliation, runtime checks and full per-operation event schemas missing. |
