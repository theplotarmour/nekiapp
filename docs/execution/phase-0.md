...
| OpenAPI/errors/auth matrix | Partial: 324 operation rows spanning all 22 plan families, route/field policies and request/error profiles. Structural validator passes. 65 identity/preference/address/geo/catalogue/discovery/bookmark operations typed in OpenAPI with 622 validated examples, 41 rejected negative schema cases and five rejected query cases; 259 operations remain untyped. Final grants, source reconciliation, runtime checks and full per-operation event schemas missing. |
