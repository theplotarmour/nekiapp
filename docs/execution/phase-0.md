...
| OpenAPI/errors/auth matrix | Partial: 324 operation rows spanning all 22 plan families, route/field policies and request/error profiles. Structural validator passes. 150 identity/discovery/organization/mission-management/moderation/contribution/checkout/case and refund operations typed in OpenAPI with 1,625 validated examples, 105 rejected negative schema cases and five rejected query cases; 174 operations remain untyped. Final grants, source reconciliation, runtime checks and full per-operation event schemas missing. |
