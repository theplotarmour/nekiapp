...
| OpenAPI/errors/auth matrix | Partial: 324 operation rows spanning all 22 plan families, route/field policies and request/error profiles. Structural validator passes. 133 identity/discovery/organization/mission-management/moderation/contribution and checkout operations typed in OpenAPI with 1,421 validated examples, 82 rejected negative schema cases and five rejected query cases; 191 operations remain untyped. Final grants, source reconciliation, runtime checks and full per-operation event schemas missing. |
