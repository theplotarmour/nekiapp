...
| G03 / NK-GAP-03 | Partial | Product + tech lead | [P1 backlog](phase-1-backlog.md) has 33 dependency-linked tickets totaling 89 estimated person-days, capacity scenarios and a cost formula; actual people, availability and rates remain unassigned. | P0 |
| G04 / NK-GAP-04 | Partial | Tech lead | All eight repositories inspected at pinned revisions; [report](../rnd/open-source-review.md) and 61 hashed evidence files delivered. Unsupported ADR-002 numerical comparison withdrawn; review and compatibility spikes remain open. | P0; NK-DOC-2 |
...
| G28 / NK-GAP-28 | Partial | Tech lead | Record bounded version/provider/deployment spikes with reproducible evidence and unavailable accounts. | P0–P1 |
...
| G30 / NK-GAP-30 | Partial | Designer | Board inspected; 64 token contrast pairs and a 320 px/200% dark 36-step prototype check pass. Complete other widths/themes, real keyboard/screen-reader checks, assets and participant review. | P0–P2 |
...
| NK-DOC-2.01 | Inspect first four named research repositories at pinned commits; record licenses, architecture/test evidence and rejected ideas. | Public source availability | Tech lead / 2 days | Static inspection delivered; upstream tests not executed |
| NK-DOC-2.02 | Inspect remaining four repositories with the same evidence; qualify unsupported ADR comparisons. | NK-DOC-2.01 format | Tech lead / 2 days | Static inspection delivered; ADR-002 evidence corrected; compatibility/review open |
...
| NK-DOC-4.02 | Reviewed OpenAPI schemas/examples, validation and generated-client workflow; reconcile TDD/schema references. | NK-DOC-4.01 | Backend / split after inventory | All 324 inventory operations have validated draft contracts; review, generated-client workflow and source reconciliation remain open |
| NK-DES-2.01 | Full route/state inventory covering consumer, volunteer, org, ops/Finance, recovery and inaccessible states. | PRD + F0–F10; G30 | Design + Flutter / 2 days | Draft delivered: 165 screen/transport rows and shared states; prototype and review evidence open |
...
|---|---|
| Scope/source/decision register | Initial register and [operating policy draft](../policies/foundation-draft.md) delivered; policy values/decisions remain open. |
...
| Research/provider/version evidence | Partial: Razorpay primary documentation inspected, retry conflict recorded, seven sandbox probes specified but not executed. All eight named repositories now have static source evidence at pinned commits, license observations and test-file inventories. The [current Flutter probe](../rnd/flutter-current-stack-spike.md) resolves/generates without the older analyzer mismatch, passes analysis and 14 tests, builds worker/web, and proves same-context Chrome IndexedDB reload/account clearing. The [PostgreSQL 17.11 outbox probe](../rnd/postgres-outbox-spike.md) passes 14 transaction/recovery tests. Mobile/other-browser/provider and PostgreSQL 16 target probes remain open; upstream reference applications were not executed. |
| Transition/event catalogue and revised schema | Partial: 139 command rows across org/volunteer/mission, contributions, money, logistics/attendance and proof/impact; event processing/consumer map drafted; ADR-014 cursor/savepoint/retry mechanism reconciled to TDD/platform model with bounded PostgreSQL probe evidence. 30 PD/FD/CD/LD/VD decisions open and 77 planned cases; accepted per-event schemas, source reconciliation and revised DDL missing. |
| OpenAPI/errors/auth matrix | Partial: 324 operation rows spanning all 22 plan families, route/field policies and request/error profiles. Structural validator passes. 315 JSON HTTP operations typed in OpenAPI with 3,592 validated examples, 257 rejected negative schema cases and five rejected query cases. One separately typed WebSocket transport has 11 validated handshake/frame examples and nine rejected cases; eight public-web transports have 18 validated examples, nine rejected cases, seven parsed HTML fixtures and an escaped-content check. Inventory schema coverage is complete; approval and runtime behavior remain unverified. Provider intake has 20 passing synthetic local HMAC/SQLite tests, not sandbox or financial-effect proof. Final grants, source reconciliation, runtime checks and full per-operation event schemas missing. |
| Design states/prototypes/assets/usability evidence | Partial: [screen/state inventory](../design/screen-state-inventory.md) has 165 rows, 12 state profiles and nine prototype journey requirements. References cover FR-01–17 and F0–F10; [interactive prototype](../design/prototype/README.md) covers nine journeys/36 steps, five recovery variants, and a checked 320 px/200% dark layout. [Tokens/assets handoff](../design/asset-handoff.md) and 64 passing contrast pairs delivered. Figma/full screen assets, remaining accessibility matrix and real formative usability evidence remain missing. |
| Operating policy drafts and approved foundation blockers | [Draft delivered](../policies/foundation-draft.md), including operating boundaries, data classes and decision separation; numerical/legal/eligibility/retention approvals remain open. |
| ADR contradictions reconciled | ADR-002/TDD select the tested Riverpod 3 baseline; ADR-014/TDD/platform schema correct cursor locking, retry recording and DLQ identity. Other schema/domain/provider contradictions remain open. |
| Sized Phase 1 backlog and staffing/cost model | [33 tickets / 89 estimated person-days](phase-1-backlog.md), dependencies, acceptance, capacity scenarios and cost formula delivered. Actual staffing/rates and dependency gate closure remain open. |
| Named external/legal/provider owners and dates | Unassigned. Live-funds approval remains a later explicit gate. |

NK-DOC-3.01 draft delivered: [catalogue](../state-machines/README.md), [open decisions and planned cases](../state-machines/review-decisions.md). Source-backed rules and proposed resolutions are distinguished; no policy approvals or runtime tests are claimed. NK-DOC-3.02 draft delivered: [contributions](../state-machines/contributions.md), [financial commands](../state-machines/payments-and-payouts.md), [money invariants](../state-machines/money-invariants.md), [25 planned cases](../state-machines/money-acceptance.md) and [provider evidence](../rnd/payment-provider-spike.md). G09–G12 are partial: proposed fixes exist, but no SQL/sandbox proof or Finance review. NK-DOC-3.03 draft delivered: [logistics/attendance](../state-machines/logistics-and-attendance.md), [proof/impact](../state-machines/proof-and-impact.md), [events/recovery](../state-machines/events-and-recovery.md) and [fulfilment checks](../state-machines/fulfilment-acceptance.md). G13/G17/G19 are partial, not proven. NK-DOC-4.01 draft delivered: [API workspace](../api/README.md). G05/G25 are partial; inventory rows do not prove an implemented endpoint or complete permission enforcement. NK-DOC-4.02 started: [typed identity contract and reproduction](../api/identity-contract.md), [exact coverage](../api/openapi-coverage.json). Strict inventory schema completeness now passes; this does not pass the P0 gate. [Discovery schema slice](../api/discovery-contract.md) delivered with public/private projection checks. [Organization and mission-management slice](../api/organization-contract.md) delivered with applicant/reviewer separation and guarded decision shapes. [Contribution and checkout slice](../api/contribution-contract.md) delivered with quote-based confirmation, independent payment recovery and checkout uncertainty. [Case and refund slice](../api/case-refund-contract.md) delivered with separate owner/Finance projections and bound approval snapshots. [Payout and Finance slice](../api/payout-contract.md) delivered with transfer uncertainty, return history and scoped reports. [Provider intake slice](../api/provider-contract.md) delivered with signature transport and 20 local fixture tests. RP-06 remains open pending sandbox evidence. [Volunteer/skills/attendance slice](../api/volunteer-attendance-contract.md) delivered with 43 operations and separate booking/assignment/evidence shapes. [Logistics slice](../api/logistics-contract.md) delivered with 24 operations and private field/custody/receipt boundaries. [Media/proof slice](../api/media-proof-contract.md) delivered with 26 operations and distinct upload, processing, participant and reviewer boundaries. [Impact slice](../api/impact-contract.md) delivered with 12 operations separating original/effective claims and conditional sharing. [Administration slice](../api/administration-contract.md) delivered with 30 scoped access/support/recovery operations. [Notification/ticket/saved-method slice](../api/engagement-contract.md) delivered with six operations and explicit snapshot/availability boundaries. [WebSocket slice](../api/realtime-contract.md) delivered with separate transport coverage and frame validation. [Public-web slice](../api/web-contract.md) delivered. [Eight-repository inspection](../rnd/open-source-review.md) delivered with verified revision/blob provenance. [Route/state inventory](../design/screen-state-inventory.md) drafted and structurally checked. [Historical Flutter probe](../rnd/flutter-stack-spike.md) is superseded for compatibility selection by the [current stack/browser evidence](../rnd/flutter-current-stack-spike.md). The checkpoint below records the next completed engineering slice and remaining P0 gates.
