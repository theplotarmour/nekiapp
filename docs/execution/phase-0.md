...
| OpenAPI/errors/auth matrix | Partial: 324 operation rows spanning all 22 plan families, route/field policies and request/error profiles. Structural validator passes. 174 identity/discovery/organization/mission-management/moderation/contribution/checkout/case/refund/payout and Finance operations typed in OpenAPI with 1,941 validated examples, 131 rejected negative schema cases and five rejected query cases; 150 operations remain untyped. Final grants, source reconciliation, runtime checks and full per-operation event schemas missing. |
